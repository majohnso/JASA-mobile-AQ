...
The R script <tt> run_all_code.R </tt> illustrates the analysis performed in ``Fine-scale spatiotemporal air pollution analysis using mobile monitors on Google Street View vehicles" using simulated data.  If the user obtains the actual Google data, <tt> run_all_code.R </tt> should recreate the analysis included in the paper. 

The following sections walk through the various components of <tt> run_all_code.R </tt>. Note that this <tt>R</tt> script and all others assume the data and scripts are all stored in a single, common folder. 
